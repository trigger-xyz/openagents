  ...
  const document = {
    ...
    promises: [
      ...
      },
      {
        ...
        safeCopy:
          'Launched for the scoped proof, and two distinct independent contributors have now been paid real Bitcoin. The public run `run.tassadar.executor.20260615` is active, and the decentralized contribution loop is proven end-to-end in the open: an independent contributor installs Pylon, claims a window lease, and submits a Tassadar executor trace; an independent validator on a separate machine/identity replays the pinned fixture and the verification challenge finalizes `Verified`. The real paid-settlement gate is met for a bounded scope and has now produced two real settled receipts, native over Spark, each with `realBitcoinMoved:true`, `moneyMovement:real_bitcoin`, and `state:settled`: a 1,000-sat owner-armed canary to the first independent contributor, and a 5-sat settlement to a second independent contributor who came through the rc.32 self-serve public install→register→claim→submit→independent-validation path. Real settled total for the run is 1,005 sats (1,000 + 5); the enumerable per-run settled feed is `GET /api/training/runs/run.tassadar.executor.20260615/settlements`. The 5-sat self-serve settlement was operator-retro-settled via the admin settlement endpoint because the auto-stream skipped at verdict time (a payout-target resolution bug, since fixed); a fully-autonomous auto-stream settlement (the gate firing at verdict with no operator action) has not happened yet. The earlier 5-sat simulation receipt remains valid historical context as a simulation-backed (`realBitcoinMoved:false`) record that proved the settlement-record/projection path — it is not real Bitcoin movement and is excluded from the 1,005 real total. These are exactly two bounded canary-scale settlements to two contributors, not network-scale paid training.',
        unsafeCopy:
          'Do not claim this is a network-scale, large, or the largest decentralized training run, that hundreds of contributors are paid, that contributors are being paid at scale, that public gradients mutate a canonical model, or that any unbounded payout authority exists. The real paid-settlement proof is exactly two bounded settlements (1,000-sat canary + 5-sat self-serve) to two distinct independent contributors (`realBitcoinMoved:true`), 1,005 sats total; copy must stay scoped to those two settlements and must not extrapolate to broad earning. Do not claim the fully-autonomous auto-stream settlement has happened (the 5-sat self-serve payout was operator-retro-settled). Do not claim the run has constructed a new model capability beyond the fixed executor workload. The prior simulation-backed receipt still does not prove real sats moved.',
        evidenceRefs: [
        ...
        blockerRefs: [],
        verification:
          'GET /api/public/training/runs/run.tassadar.executor.20260615: run state active, summary.metrics.qualifiedContributorCount >= 1, and Verified exact_trace_replay verification challenges with settlement_recorded receipts linked to the run. GET /api/training/runs/run.tassadar.executor.20260615/settlements enumerates the per-run settled rows: it returns exactly two `realBitcoinMoved:true` rows (1,000-sat canary to the first contributor and 5-sat self-serve to a second contributor, 1,005 real total) plus one `realBitcoinMoved:false` simulation row that is excluded from the real total. The real paid-settlement gate is satisfied by receipt receipt.nexus.tassadar_run_settlement.idempotency.tassadar.run_settlement.5b7f92fe.canary1k.v6.20260618 (backed by challenge training.verification.challenge.071445c5-6ad6-4136-87e3-253b01914b4c) and receipt receipt.nexus.tassadar_run_settlement.settlement.tassadar.retro.10c3b01b.trigger.v1 (backed by challenge training.verification.challenge.10c3b01b-c781-4a03-a8ed-4ae6c6195fe4), both realBitcoinMoved:true, moneyMovement:real_bitcoin, state:settled, adapter:spark_treasury. The 5-sat self-serve settlement is operator-retro-settled, not a fully-autonomous auto-stream. The simulation receipt remains only settlement-record/projection evidence (`realBitcoinMoved:false`) and must not be counted as real Bitcoin movement, accepted-work sats settled, or spendable contributor payout. Note: the run-summary `settledPayoutSats` field and the `/api/public/pylon-stats` 24h aggregate still read 1,010 because they have not yet excluded the simulation row; the `/settlements` feed is the reconciled per-run real total (1,005) and that aggregate reconciliation is tracked separately. Broadening beyond these two bounded settlements requires more public settled receipts with `realBitcoinMoved:true`, a participant-count methodology, and accepted-work receipts for more contributors.',
        authorityBoundary:
          'A launched, verified loop plus two bounded real settlements (1,000-sat canary + 5-sat self-serve, 1,005 sats total) to two distinct independent contributors does not authorize network-scale training claims, paid-at-scale or hundreds-paid claims, contributor admission beyond the published rules, largest-run comparisons, canonical-checkpoint mutation from public gradients, fully-autonomous auto-stream settlement claims (the 5-sat payout was operator-retro-settled), or any spend beyond these two bounded operator-approved settlements. The earlier simulation-backed settlement record proves the projection path only, not real movement.',
      },
      {
        ...basePromiseFields,
      ...
      },
      {
        ...
        safeCopy:
          'This is the decentralized-optimizer lane (RESEARCH_PLAN W5). Public Pylons may eventually contribute bounded, verified training windows to Psion/Tassadar student models; candidate updates enter quarantine first and can promote only after recompute/replicate, canary, and explicit promotion gates pass. The H1 gate and frozen-core validator are code-backed, but today public devices do generation, validation, and evaluation only; no public contributor gradient window has been accepted, promoted, paid, or settled.',
        unsafeCopy:
      ...
      },
      {
        ...
        safeCopy:
          'Source support and admitted-Mac smoke evidence now exist for the basic local Apple FM mode: on supported macOS Apple Silicon machines, Autopilot Desktop can ask Pylon to start a bounded local Apple FM chat/tool session after live readiness passes, using read-only workspace tools and public-safe desktop event summaries without sending prompts to OpenAgents hosted compute. This is yellow, not green: the current public installer still needs a signed recut with helper launch/supervision, and the claim is limited to basic local chat/tool use on admitted Apple hardware.',
        unsafeCopy:
